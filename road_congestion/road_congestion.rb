...
class RoadLoadChecker
  def initialize(array_of_dots)
    @array_car_position = array_of_dots
  end
  ...
  def calculate_distance_between_car
    ...
    @array_car_position.each_index do |i|
      if i < @array_car_position.size - 1
        distance_between_car.push((@array_car_position[i] - @array_car_position[i + 1]).abs)
      else
        break
      end
    end
